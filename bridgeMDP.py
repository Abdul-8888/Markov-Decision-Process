...
class bridgeMDP:
    ...
    def startState(self, x, y):
        self.startState = self.getStateNo(x,y)
    ...
    def takeAction(self,s,a):
        ...
        if a == 'up':
            return self.getStateNo(x,y-1)
        elif a == 'down':
            return self.getStateNo(x,y+1)
        elif a == 'left':
            return self.getStateNo(x-1,y)
        elif a == 'right':
            return self.getStateNo(x+1,y)
        else:
            return None
...
def policyEvaluation(mdp: bridgeMDP, policy):
    ...
    for i in range(1,100):
        for s in states:
            c = 0.0
            for n in states:
                c += mdp.transitionProbabilities(s,policy,n) * (mdp.reward(s,policy,n) + (mdp.discount()*(p_v[n])))
